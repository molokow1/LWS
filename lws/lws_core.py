...
class LWSCore():
    def __init__(self, global_config, override_global_config = False, new_config = None):
        ...
        self.base_station = BaseStation('B0', global_config.bsX, global_config.bsY, 0, global_config, env=self.env)
    ...
    def init_end_devices(self, node_pos_arr = None):
        ...
        if node_pos_arr == None:
            node_pos_arr = generate_random_end_device_positions(self.num_end_devices, self.global_config.maxDist, self.global_config.bsX, self.global_config.bs_y)

        if self.global_config.enableACKTest:
            limit = int(self.num_end_devices * self.global_config.ACKPercentage)
        else:
            limit = 0
        ...
        for i, pos in enumerate(node_pos_arr):
            ...
            self.end_devices.append(EndDevice(device_id=i, x=pos[0], y=pos[1], dist=pos[2], global_config=self.global_config, pkt_type=pkt_type, env=self.env))
